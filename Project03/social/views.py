...
def change_view(request):
    ...
    context = {'user': request.user,
               'change_form': form, 'user_info': user_info}
...
def change_user_view(request):
    ...
    if request.method == 'POST':
        ...
        u_int = request.POST.get('interest')
        if models.Interest.objects.filter(label__iexact=u_int):
            int_instance = models.Interest.objects.filter(label__iexact=u_int)
        else:
            int_instance = models.Interest.objects.create(label=u_int)
        #
...
def people_view(request):
    ...
    if request.user.is_authenticated:
        ...
        for f_r in models.FriendRequest.objects.filter(to_user=t).all():
            friend_requests += [f_r.from_user]
        l = list(models.FriendRequest.objects.filter(
            from_user=user_info).all())
        for i in l:
            dis += [i.to_user]
        context = {'user_info': user_info,
